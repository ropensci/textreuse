...
head(minhash(c("turn some tokens", "some tokens to", "tokens to hashes")))
```

Now when we load our corpus, we will tokenize our texts as usual, but we will use our generated `minhash()` function to generate the hashes.
...
Now we can use the `lsh()` function to calculate the locality-sensitive hashes into a cache. Note that we are creating the cache for the first time. But if we pass an existing cache and (this is important) use the same minhash function and the same number of bands, we can pass an existing cache to the `lsh()` function with the `buckets = ` parameter. That way, we can continue to add new documents without having to recalculate the LSH signatures for the whole corpus. For a 640 document, 60 million word corpus, calculating the cache takes seconds, compared to hours of pairwise comparisons.
...
Notice that LSH has identified the same four pairs of documents as potential matches that we found with pairwise comparisons, but did so much faster. But we do not have similarity scores; we only know that these documents are likely to have Jaccard similarity scores above the `r round(lsh_threshold(h = 240, b = 80), 3)` threshold.

We could compute Jaccard similarities using the minhashes, or we could retokenize and hash only the candidate documents. We will do the later, using the `lsh_subset()` function to get just the documents that we want and the `tokenize()` function to retokenize all the documents in the subset corpus.

```{r}
subset <- lsh_subset(candidates)
corpus_subset <- corpus[subset]
corpus_subset <- tokenize(corpus_subset, tokenizer = tokenize_ngrams, n = 5)
```

Now we can use `lsh_compare()` to apply a similarity function to the candidate pairs of documents. Note that we only have to do 4 comparisons for all the candidates, instead of 45 when comparing all of the documents in the corpus pairwise.
